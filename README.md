...
The goal is to build the control stack in small, testable layers: represent a requested vehicle motion, make it safe, serialize it for communication, then connect that software to the Raspberry Pi, Arduino, steering servo, motor controller, and sensors.

> **Current status:** software foundation in progress. Hardware communication and physical vehicle control are planned next.
...
- a small executable used to exercise the command-validation path.
...
That separation is intentional: later code can request a motion without being responsible for enforcing the vehicle's allowed command range.
...
- [`src/vehicle_command.cpp`](https://github.com/Brighht/ackermann-lab-/blob/main/src/vehicle_command.cpp) — applies the current speed and steering safety limits.
- [`src/main.cpp`](https://github.com/Brighht/ackermann-lab-/blob/main/src/main.cpp) — exercises the current command pipeline.
...
| 2 | Vehicle command representation and validation | 🟡 In progress |
| 3 | Serialize commands for Pi → Arduino communication | ⏳ Next |
| 4 | Establish Raspberry Pi ↔ Arduino serial link | ⏳ Planned |
